...
% Для какого алгоритма строим временные зависимости?
calc_settings.alg_num = 13;
calc_settings.alg_count = 18; % Количество алгоритмов ОМП

% Параметры КЗ
sc_settings.position_init = 110; % Местоположение КЗ
sc_settings.faulted_phase = 1; % Поврежденная фаза
sc_settings.period_of_fault = 3; % В конце какого периода устраивается КЗ
...
graphs_settings.xlim_min = 1800;
graphs_settings.xlim_max = 5000;
graphs_settings.ylim_min = 0;
graphs_settings.ylim_max = 90;
graphs_settings.algs_for_plot = [6 11 13 17 18]; % Алгоритмы для которых строим графики
graphs_settings.marker_size = 20;
graphs_settings.markers = ["Square" "Diamond" "o" "*" "+"];
graphs_settings.result_directory = "graphs";  

% Расположение исходных данных и результатов
directories_settings.PMU_data_MAT = 'PMU_data_MAT';
directories_settings.PMU_data_CSV = 'PMU_data_CSV';
directories_settings.results_data = 'results';
...
function [delta_percent] = calc_algs_error_one_set_PMU(...
    ...
    if factor ~= "load" && factor ~= "sc_phase" && factor ~= "ef_mutual_angle"
        switch sc_settings.period_of_fault
            case 1
                PMU_set = fault_inception_moment + period;
            case 2
                PMU_set = fault_inception_moment + 2*period;
            case 3
    ...
    for idx_exp = 1:exp_count
        ...
        if factor == "load" || factor == "sc_phase" || factor == "ef_mutual_angle"
            switch sc_settings.period_of_fault
                case 1
                    PMU_set = fault_inception_moment(idx_exp) + period;
                case 2
                    PMU_set = fault_inception_moment(idx_exp) + 2*period;
                case 3
                    PMU_set = fault_inception_moment(idx_exp) + 3*period;
            end
        end
